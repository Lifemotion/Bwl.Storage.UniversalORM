...
Public Class ExamplesApp
    ...
    Private Sub FirebirdStorageExample() Handles buttonFirebirdStorageExample.Click
        ...
        Dim objSearchResults1b = fbStorageForTestData.FindObj(New SearchParams({New FindCriteria("Cat", FindCondition.equal, "cat1")}))
        'поиск идентификаторов объектов, где поле Cat содержит cat
        Dim objSearchResults1c = fbStorageForTestData.FindObj(New SearchParams({New FindCriteria("Cat", FindCondition.likeEqual, "cat")}))
        'поиск идентификаторов объектов, где поле Second вложенного объекта NestedData равно 1
        Dim objSearchResults1d = fbStorageForTestData.FindObj(New SearchParams({New FindCriteria("NestedData.Second", FindCondition.equal, 1)}))
