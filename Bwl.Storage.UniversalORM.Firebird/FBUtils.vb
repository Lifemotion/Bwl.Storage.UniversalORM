    ...
    Public Shared Sub ExecSQL(connString As String, sql As String, Optional parameters As FirebirdSql.Data.FirebirdClient.FbParameter() = Nothing)
        ...
        Catch ex As Exception
            Dim readableParams = If(parameters IsNot Nothing AndAlso parameters.Any(),
                                    ...
                                    "None")
            Throw New Exception($"FbUtils.ExecSQL. Connection string: {connString}{vbNewLine}SQL: {sql}{vbNewLine} Params: {readableParams}{vbNewLine} ERR: {ex.ToString()}")
        Finally
            con.Close()
            con.Dispose()
        End Try
    ...
    Public Shared Function ExecSQLScalar(connString As String, sql As String, Optional parameters As FbParameter() = Nothing) As Object
        ...
        Catch ex As Exception
            Dim readableParams = If(parameters IsNot Nothing AndAlso parameters.Any(),
                                    parameters.Select(Function(f) $"{f.ParameterName}, type {f.DbType}, value {f.Value}").Aggregate(Function(f, t) f + vbNewLine + t),
                                    "None")
            Throw New Exception($"FbUtils.ExecSQLScalar. Connection string: {connString}{vbNewLine}SQL: {sql}{vbNewLine} Params: {readableParams}{vbNewLine} ERR: {ex.ToString()}")
        Finally
            con.Close()
            con.Dispose()
        End Try
    ...
    Public Shared Function GetObjectList(connString As String, sql As String, Optional parameters As FbParameter() = Nothing) As List(Of List(Of Object))
        ...
        Catch ex As Exception
            Dim readableParams = If(parameters IsNot Nothing AndAlso parameters.Any(),
                                    parameters.Select(Function(f) $"{f.ParameterName}, type {f.DbType}, value {f.Value}").Aggregate(Function(f, t) f + vbNewLine + t),
                                    "None")
            Throw New Exception($"FbUtils.GetObjectList. Connection string: {connString}{vbNewLine}SQL: {sql}{vbNewLine} Params: {readableParams}{vbNewLine} ERR: {ex.ToString()}")
        Finally
