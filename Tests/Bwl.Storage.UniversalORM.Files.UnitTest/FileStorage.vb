...
<TestClass()> Public Class FileStorage
	...
	Public Sub FileStorage_FindObj_SortParam()
		...
		Dim sortp = New SortParam("Cat", SortMode.Ascending)
		sp.SortParam = sortp
		Dim p2 = _stor.FindObj(sp)

		sortp = New SortParam("Cat", SortMode.Descending)
		sp.SortParam = sortp
		...
		Assert.AreEqual(p2(0), _data2.ID)
		Assert.AreEqual(p2(1), _data4.ID)

		Assert.AreEqual(p3(0), _data5.ID)
		Assert.AreEqual(p3(1), _data4.ID)
	End Sub
